...
import networkingClass.NetworkingManager;
...
public class MainGameLoop {
	...
	//	Entity treee =  new Entity(trees, new Vector3f(200,0,150),0,0,0,10);
		...
		TexturedModel staticModelw = new TexturedModel(model,new ModelTexture(loader.loadTexture("wizard")));//
		TexturedModel staticModel2 = new TexturedModel(model,new ModelTexture(loader.loadTexture("playerTexture2")));
		Player player =  new Player(staticModel, new Vector3f(10,0,270),0,0,0,2); //2 represents the size of entity 80,0,70),0,0,0,5
		Entity rival =  new Entity(staticModel2, new Vector3f(30,0,310),0,0,0,2); //2 represents the size of entity 80,0,70),0,0,0,5
		Entity wizard =new Entity(staticModelw, new Vector3f(100,0,100),0,0,0,4);
		...
		entities.add(new Entity(staticModelt, new Vector3f(195,0,90),0,0,0,13));
		entities.add(new Entity(trees, new Vector3f(198,50,90),0,0,0,3));
		...
		List<GuiTexture> rule  = new ArrayList<GuiTexture>();
		GuiTexture rules = new GuiTexture(loader.loadTexture("rule"), new Vector2f(0f,0f), new Vector2f(1.0f, 1.0f));
		rule.add(rules);
		
		List<GuiTexture> rulee  = new ArrayList<GuiTexture>();
		GuiTexture ruless = new GuiTexture(loader.loadTexture("rule2"), new Vector2f(0f,0f), new Vector2f(1.0f, 1.0f));
		rulee.add(ruless);
		...
		while(!Keyboard.isKeyDown(Keyboard.KEY_M)){
			...
			while(!Keyboard.isKeyDown(Keyboard.KEY_N)){
				
				guiRenderer.render(rule);
				DisplayManager.updateDisplay();
			}
				guis.clear();
				guiRenderer.render(guis);
			
			while(!Keyboard.isKeyDown(Keyboard.KEY_S)){
					
				guiRenderer.render(rulee);
				DisplayManager.updateDisplay();
			}
				guis.clear();
				guiRenderer.render(guis);
			...
			wizard.increaseRotation(0, 1, 0);
			camera.move();
			player.move();
	
		
			Vector3f[] arr= nn.PositionUpdate(player.getPosition(), player.getRotation());
	...
	//		if  (Keyboard.isKeyDown(Keyboard.KEY_B) & Keyboard.isKeyDown(Keyboard.KEY_N) ) {
			...
			while ( player.getRotZ() == 100 || Keyboard.isKeyDown(Keyboard.KEY_0) ) { 
				List<GuiTexture> wizardd  = new ArrayList<GuiTexture>();
				GuiTexture wizarddd = new GuiTexture(loader.loadTexture("secret"), new Vector2f(0f,0f), new Vector2f(1.0f, 1.0f));
				wizardd.add(wizarddd);
				guiRenderer.render(wizardd);
				DisplayManager.updateDisplay();
			}guis.clear();
			guiRenderer.render(guis);
			...
			while ( Keyboard.isKeyDown(Keyboard.KEY_Y) ) { 
				List<GuiTexture> won  = new ArrayList<GuiTexture>();
				GuiTexture wonn = new GuiTexture(loader.loadTexture("won"), new Vector2f(0f,0f), new Vector2f(1.0f, 1.0f));
				won.add(wonn);
				guiRenderer.render(won);
				DisplayManager.updateDisplay();
